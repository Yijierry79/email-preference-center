...
var swiperOptionsIndex = {
    ...
    pagination: {
        ...
        renderBullet: function (index, className) {
            // index 是当前分页项的索引 (0, 1, 2...)
            // className 是 Swiper 默认的类名 (swiper-pagination-bullet)
            return '<span class="' + className + '">' + titles[index] + '</span>';
        },
    },
    navigation: {
        nextEl: '.js_banner .swiper-button-next',
        prevEl: '.js_banner .swiper-button-prev',
    },
    on: {
        ...
        transitionStart:function(){
            if ($('.js_banner .swiper-slide .text h3 span').length) {
                gsap.set(".js_banner .swiper-slide .text h3 span", {
                    opacity: 0,
                    transform: 'translateY(25px)',
                });
                gsap.set(".js_banner .swiper-slide .text .p", {
                    opacity: 0,
                    transform: 'translateY(25px)',
                });
                setTimeout(function () {
                    gsap.to(".js_banner .swiper-slide-active .text h3 span", {
                        opacity: 1,
                        transform: 'translateY(0)',
                    })
                    gsap.to(".js_banner .swiper-slide-active .text .p", {
                        opacity: 1,
                        transform: 'translateY(0)',
                    });
                }, 40)
...
var banner_swiper_index = new Swiper(".js_banner .swiper", swiperOptionsIndex);
banner_swiper_index.autoplay.stop(); // 停止轮播

// 确保所有视频处于暂停状态
$('.js_banner video').each(function () {
    this.pause();
    this.currentTime = 0;
});
